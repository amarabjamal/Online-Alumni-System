...
$number_of_result = $conn->query('SELECT count(*) from users, statuses WHERE users.status_id = statuses.id AND statuses.status = "Pending"')->fetchColumn(); 
            ...
            <table class="table table-striped table-hover table-bordered container" id="mainTable">
                ...
                <tbody id="myTable">
                    ...
                    <?php
                        ...
                        try {
                            $query = "SELECT * FROM users, faculties, statuses WHERE (users.fac_id = faculties.id AND users.status_id = statuses.id) AND statuses.status = \"Pending\" order by users.id asc
                ...
                <nav aria-label="Page navigation">
                        ...
                        <?php if($page > 1) {
                            echo '<li class="page-item"><a class="page-link" href = "pending.php?page=' . $current_page - 1 . '" tabindex="-1">Previous</a></li>';  
                        } 
                        for($page = 1; $page<= $number_of_page; $page++) { 
                            if($page == $current_page) {
                                echo '<li class="page-item active"><a class="page-link" href = "pending.php?page=' . $page . '">' . $page . ' <span class="sr-only">(current)</span></a></li>';  
                            } else {
                                echo '<li class="page-item"><a class="page-link" href = "pending.php?page=' . $page . '">' . $page . ' </a></li>';  
                            }
                        } 
                        if($number_of_page > 1 && $current_page != $number_of_page) {
                            echo '<li class="page-item"><a class="page-link" href = "pending.php?page=' . $current_page + 1 . '">Next</a></li>';  
                        } 
                        ?>
                        </ul>
                </nav>
